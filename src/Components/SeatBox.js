...
function SeatBox({ imgSrc, title, price, sku, product_id, isClicked}) {

  const notifyAndSubmit = (e) => {
    console.log("target->", e.target.dataset.id, e.target.dataset.name)
  
    toast.success(e.target.dataset.name + '切换成功', {
      position: "top-center",
      autoClose: 500,
  ...
  return (
    <div className="details">
      ...
      <div className="cart-btn">
        ...
        <button onClick={notifyAndSubmit} className="btn" data-name={title} data-id={product_id}>切换该桌</button>
      </div>
      <ToastContainer />
    </div>
  );
